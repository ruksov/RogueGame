...
public class RoomNodeSO : ScriptableObject
{
    ...
    [FormerlySerializedAs("roomNodeType")] public RoomNodeTypeSO Type;
    ...
    public void Draw(GUIStyle nodeStyle)
    {
        ...
        if (parentRoomNodeIDList.Count > 0 || Type.isEntrance)
        {
            // Display a label that can't be changed
            EditorGUILayout.LabelField(Type.roomNodeTypeName);
        }
        else
        {
            // Display a popup using the RoomNodeType name values that can be selected from (default to the currently set roomNodeType)
            int selected = roomNodeTypeList.list.FindIndex(x => x == Type);

            int selection = EditorGUILayout.Popup("", selected, GetRoomNodeTypesToDisplay());

            Type = roomNodeTypeList.list[selection];
    ...
    public bool IsChildRoomValid(string childID)
    {
        ...
        foreach (RoomNodeSO roomNode in roomNodeGraph.Nodes)
        {
            if (roomNode.Type.isBossRoom && roomNode.parentRoomNodeIDList.Count > 0)
                isConnectedBossNodeAlready = true;
        }

        // if the child node has a type of boss room and there is already a connected boss room node then return false
        if (roomNodeGraph.GetRoomNode(childID).Type.isBossRoom && isConnectedBossNodeAlready)
            return false;

        // If the child node has a type of none then return false
        if (roomNodeGraph.GetRoomNode(childID).Type.isNone)
            return false;
        ...
        if (roomNodeGraph.GetRoomNode(childID).Type.isCorridor && Type.isCorridor)
            return false;
        ...
        if (roomNodeGraph.GetRoomNode(childID).Type.isCorridor && childRoomNodeIDList.Count >= Settings.maxChildCorridors)
            return false;

        // if the child room is an entrance return false - the entrance must always be the top level parent node
        if (roomNodeGraph.GetRoomNode(childID).Type.isEntrance)
            return false;

        // If adding a room to a corridor check that this corridor node doesn't already have a room added
        if (!roomNodeGraph.GetRoomNode(childID).Type.isCorridor && childRoomNodeIDList.Count > 0)
            return false;
